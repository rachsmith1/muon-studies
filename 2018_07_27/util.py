...
class StackedPlot:
    def __init__(self, title, xAxisTitle, yAxisTitle, binning, normBool, logBool):
        #title of stacked plot
        self.title      = title
        #x-axis title of stacked plot
        self.xAxisTitle = xAxisTitle
        #y-axis title of stacked plot
    ...
    def printPlot(self):
        ...
        allHisto  = [list(histo[0]) for histo in self.histoList]
        allColor  = [histo[1]["color"] for histo in self.histoList]
        allLabel  = [histo[1]["name"] for histo in self.histoList]
        allBin    = [self.binning for histo in self.histoList ]
        allWeight = [[1/ntotal]*len(histo[0]) for histo in self.histoList]
...
class BarPlot:
    ...
    def printPlot(self):
        ...
        for i, v in enumerate(self.data):
            ntotal += v
        for i, v in enumerate(self.data):
            plt.text(i - .25, ntotal*0.02, "{}%".format(round(v/ntotal*100, 2)), color="white")
